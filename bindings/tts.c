...
static iotjs_tts_t* iotjs_ttswrap_create(const jerry_value_t value) {
  iotjs_ttswrap_t* tlswrap = IOTJS_ALLOC(iotjs_ttswrap_t);
...
static void iotjs_ttswrap_destroy(iotjs_ttswrap_t* tts) {
  IOTJS_VALIDATED_STRUCT_DESTRUCTOR(iotjs_tlswrap_t, tts);
  iotjs_jobjectwrap_destroy(&_this->jobjectwrap);
  IOTJS_RELEASE(tts);
}

S_FUNCTION(TtsConstructor) {
  DJS_CHECK_THIS();
...
void InitTts(jerry_value_t exports) {
  ...
  iotjs_jval_set_property_jval(tls, "TtsWrap", ttsConstructor);
