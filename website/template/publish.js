...
function buildNav(members) {
  ...
  nav = nav.concat(buildYodaOS(linkto))
  nav = nav.concat(buildMemberNav(members.tutorials, "Guides", seenTutorials, linktoTutorial))
  ...
  if (items && items.length) {
    ...
    items.forEach(function(item) {
      ...
      if (!hasOwnProp.call(itemsSeen, item.longname)) {
        if (!!conf.default.useLongnameInNav || item.kind === 'namespace') {
...
exports.publish = function(taffyData, opts, tutorials) {
  ...
  var apilevel = readProjectInfo().manifest.apilevel
  opts.readme = opts.readme.replace('<!-- {project.manifest.apilevel} -->', [
    '<blockquote>',
      `<p>API Level: ${apilevel}</p>`,
    '</blockquote>',
  ].join('\n'));
