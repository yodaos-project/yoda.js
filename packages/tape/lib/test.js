...
function Test (name_, opts_, cb_) {
  ...
  this._timeout = args.opts.timeout || global.__tape_timeout__
...
Test.prototype.comment = function (msg) {
  ...
  msg.trim().split('\n').forEach(function (aMsg) {
    that.emit('result', aMsg.trim().replace(/^#\s*/, ''))
  })
}
...
Test.prototype._assert = function assert (ok, opts) {
  ...
  if (!ok) {
    ...
    var dir = __dirname + path.sep // eslint-disable-line no-path-concat
