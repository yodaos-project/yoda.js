...
test('skill event: cut skill -> scene skill', t => {
  t.plan(5)
  var count = 0
  eventBus.on('execute:appid2-cut', (dt, type) => {
    if (count === 0) {
      t.equal(dt.data.appId, 'appid2-cut')
      t.equal(type, 'frontend')
      t.pass('appid2-cut emit start')
    }
  ...
  var cutskill = manager.skills[0]
  t.equal(cutskill.paused, false)
  t.equal(cutskill.shouldEndSession, false)
  mockAction.appId = 'appid2-scece'
  mockAction.response.action.form = 'scece'
  mockAction.response.action.shouldEndSession = false
  var tts = {
    "type": "voice",
    "action": "PLAY",
