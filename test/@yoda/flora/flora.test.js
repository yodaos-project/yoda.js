...
test('module->flora->client: rpc call self', { timeout: 10 * 1000 }, t => {
  ...
  var recvCount = 0
  var methodName = 'foo'
  agent.declareMethod(methodName, (msg) => {
    ++recvCount
    return { retCode: 0, msg: [ recvCount ] }
  })
  agent.start()
...
test('module->flora->client: rpc call target', { timeout: 10 * 1000 }, t => {
  ...
  agentInfo[0].agent.declareMethod(methodName, (msg) => {
    ++agentInfo[0].invokeCount
    return { retCode: 0 }
  })
  agentInfo[1].agent.declareMethod(methodName, (msg) => {
    ++agentInfo[1].invokeCount
    return { retCode: 0 }
  })
  agentInfo[2].agent.declareMethod(methodName, (msg) => {
