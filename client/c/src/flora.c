...
static void _flora_method_call_harbor(const char *name, caps_t msg,flora_call_reply_t reply, void *arg) {
...
YODAOS_API_LOCAL struct yodaos_api_s *yodaos_get_apihd(YODAOS_APINAME api) {
    ...
    for (int i = 0; i < sizeof(api_container)/sizeof(struct yodaos_api_s); i++) {
...
YODAOS_API_PUBLIC int32_t yodaos_api_call_ignoreresult(YODAOS_APINAME api, char *params) {
    int32_t ret = 0;
    flora_call_result *data = yodaos_api_call(api, params, YODAOS_API_DEFAULT_TIMEOUT);
...
YODAOS_API_PUBLIC int32_t yodaos_ev_subscribe(YODAOS_EVNAME evName, char *params, uint32_t timeout) {
    ...
    sprintf(args, "{\"namespace\":\"%s\", \"event\":\"%s\", \"params\":%s}", ev->nameSpace, ev->evName, toset);
    caps_write_string(cmsg, (const char*)args);

    RKLogv("Subscribe cmd：%s\n", args);

    pthread_mutex_lock(&flora_lock);
    flora_agent_call(agent, "yodaos.fauna.subscribe", cmsg, "runtime", &result, timeout);
    pthread_mutex_unlock(&flora_lock);
    ret = result.ret_code;
    RKLogd("Subscribe Get retCode:%d, %s\n", result.ret_code, yodaos_api_get_result(&result));
...
YODAOS_API_LOCAL void yodaos_send_runtime_active() {
    send_runtime_msg("active", flora_send_null_cb);
}

YODAOS_API_PUBLIC int32_t yodaos_flora_post(const char *name, caps_t msg, uint32_t msgtype) {
    int32_t ret;
